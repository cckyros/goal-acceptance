...
# {{name}}:skill
name: {{skillDir}}
description: Acceptance-criteria-driven goal completion for the {{brand}} plugin — lock criteria before implementing, track task status, validate with real evidence, and confirm by an independent reviewer. Trigger words: {{name}}, acceptance criteria, goal acceptance, goal planning
...
Set values with `npx {{name}} config set <key> <value>` (project) or with
